...
\begin{frame}[c]{Volatility Affects Expected Returns Through Two Channels}
  ...
  \begin{enumerate}
    \item Investors' willingness to tolerate high-volatility to get high expected returns.
        \medskip
      \begin{itemize}
          \item This measured by the \emph{market return risk price} --- $\kappa$.
      \end{itemize}
%
      \bigskip
%
        ...
        \medskip
      \begin{itemize}
          \item This is measured by the \emph{volatility risk price} --- $\pi$.
      \end{itemize}
  \end{enumerate}
%
      \bigskip \pause
...
%
  ...
  \begin{block}{}
      $\pi$ is \emph{weakly-identified}. We develop robust methods to create \emph{uniformly-valid confidence sets} regardless of the identification strength.
  \end{block}
...
\begin{frame}[c]{Literature Review}
    ...
    \emph{Analysis between Risk and Return:} 

    \begin{itemize}
        \item  The empirical literature estimating  $\kappa$ is inconclusive, \parencite{lettau2010measuring}. 
%
...
%
    ...
    \emph{Weak Identification Literature:}
...
%
  \begin{minipage}{.49\textwidth}
    ...
    \begin{enumerate}
        \item $\pi$ is not identified if $C(\kappa - 1) = C(\kappa)$.
        \item $C(\kappa - 1) - C(\kappa) $
        \item[=] $\psi - \frac{1 - \phi^2}{2} (2 \kappa -1 )$

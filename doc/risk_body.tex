...
%
\begin{equation}
    QLR_{b}^{\ast }(\theta _{0}) \coloneqq T\widehat{g}_b(\theta _{0})^{\prime }\widehat{\Sigma }(\theta _{0},\theta _{0})^{-1}\widehat{g}_b(\theta _{0})-\underset{\theta \in \Pi }{\min }Tg_{b}^{\ast }(\theta )^{\prime }\widehat{\Sigma } (\theta ,\theta )^{-1}g_{b}^{\ast }(\theta ).
\end{equation}
%
...
We now report the estimates and confidence intervals for the reduced-form parameters $c, \delta$, and $\rho$. The confidence intervals reported here use the Gaussian limiting theory, i.e., the point estimates $\pm 1.96$ standard errors. We first obtain confidence intervals for $\log(c)$ and $\log(c) + \log(\delta)$, and transform them into confidence intervals for $c$ and $\delta$.  We create the confidence interval for $\rho$ by inverting  the interval for $\mathrm{logit}(\rho)$.
...
\begin{figure}[htb]
	...
	\includegraphics[width=.5\textwidth]{confidence_region_500.pdf}
\end{figure}


The results in \cref{tbl:structural_param_estimates} and \cref{fig:confidence_region} have a few notable features. First, we can reject the null hypothesis $\phi = 0$ at the \SI{5}{\percent} level. We cannot, however, reject the null hypotheses that $\pi = 0$ or that $\kappa = 0$. Although the projection procedure leads to conservative subvector confidence sets whose coverage could be larger than \SI{95}{\percent}, the confidence set in \cref{fig:confidence_region} appear to be quite informative. The risk price associated with the market return lies in $(0.00, 2.67)$ with at least \SI{95}{\percent} probability. The volatility risk price lies in $(-10.67, 0.00))$ with at least \SI{95}{\percent} probability. Our estimates for both parameters are reasonable given other values that previous authors have found, especially the ones that are larger in magnitude. \Textcite{han2018leverage} preferred a value of $\pi = -10$, for example. Surprisingly, we cannot  reject zero for either value.

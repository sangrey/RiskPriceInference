...
\Cref{sec:simulation} show that the method works well in simulation, and \Cref{sec:empirical} applies the methods to data on the S\&P 500 providing estimates of the risk prices. \Cref{sec:conclusion} concludes. 
...
%
  \toprule
  & {AR} & {Traditional} & {Proposed} & {AR} & {Traditional} & {Proposed} \\
  \midrule
   \multicolumn{7}{c}{Parameter Values used by \textcite{han2018leverage}} \\
  \midrule
  $\phi$ & \multicolumn{3}{c}{$T$ = 2,000} & \multicolumn{3}{c}{$T$ = 10,000} \\
  \midrule
  -0.01  & 5.0  & 2.4   & 5.2   & 4.6 & 1.7 & 5.6   \\
  -0.10  & 5.3  & 3.3   & 6.7   & 4.4 & 2.6 & 6.2   \\
  -0.40  & 6.3  & 3.3   & 5.3   & 4.9 &     &       \\
  \bottomrule
...
Consider the results reported in  \cref{tbl:test_performance}. The $AR$-statistic is robust, as we expect. The standard $QLR$-statistic appears to be conservative. The proposed test, on the other hand, is robust and has the proper size regardless of the value of $\phi$. This behavior of the proposed test is exactly what the theory predicts should happen. We see similar performance with $T = 10,000$. 

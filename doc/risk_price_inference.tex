...
Before we consider the expectation of $\widetilde{r}_{t+1}$ and show that it equals zero, consider the expectation of $\exp(m_{t,t+1}, r_{t+1})$.
%
...
%
    \intertext{The $m_{t,t+1}$ and $r_{t+1}$ variables are jointly conditionally Gaussian, and so their sum is as well. Using the formula for a log-Gaussian random variable gives} 
%
    \label{eqn:excess_return_decomposition_a}
    &= \E[m_{t,t+1}] + \E[r_{t+1}] + \frac{1}{2} \Var[m_{t,t+1}] + \frac{1}{2}\Var[r_{t+1}] - \Cov[m_{t,t+1}, r_{t,t+1}] 
\end{align}

If $r_{t+1}$ was risk-less, then since $r_{t+1}$ is an excess-return, $\E[r_{t+1}] = 0$, and all of the variance terms drop out.
This gives
%
\begin{equation}
...
Clearly, $\E[\widetilde{r}_{t+1}]= r_{t+1} + \frac{1}{2} \Var[r_{t+1} \sigma^2_{t}, \sigma^2_{t+1}] - \Cov[m_{t,t+1}, \sigma^2_{t+}]$ has mean zero.
Because the mean of $\tilde{r}_{t+1}$ given $\sigma^2_t$ does not change when we condition on $\sigma^2_{t+1}$ we can apply the \Ito\ Isometry.

...
\begin{defn}{Continuous-Time DGP}
    ...
    \begin{align}
        \dif p(t) &= \mu_{p}(t) \dif t + \psi(t) \mu_{\sigma}(t) \dif t + \sigma(t) \phi(t) \dif W_{p}(t)  +
        \sigma(t) \sqrt{ 1 - \phi^2(t)} \dif W_{p}(t)  \\
...
\purple{There is an issue from getting from above to here.}
...
\begin{alignat}{2}
    ...
    & && - \beta_{\PP, \tau}'(0) u\sigma^2_{\tau-} + O_p(\beta_{\PP, \tau}''(0)) u^2 \sigma^2_{\tau-} -
      ...
      \intertext{We can combine the 2nd derivatives with respect to $\beta$ and $\alpha$ because the volatility
          must be predictable, and so $\sigma^2(\tau-)$ and $\sigma^2(\tau)$ coincide.} 
    &= && - \alpha_{\PP, \tau}'(0) u \sigma^2_{\tau}  - \alpha_{\PP, \tau}''(0) \sigma^2_{\tau} u^2 - \beta_{\PP,
      \tau}'(0) u\sigma^2_{\tau} - \gamma_{\PP, \tau}'(0)u  - \gamma_{\PP, \tau}'' u^2 
\end{alignat}
...
\purple{This does not belong here!!!!}
To further understand the role that $\psi$ plays consider the following analogue to the third equation on p.15 of 
\textcite[p.15]{khrapov2016affine}.

Intuitively, $\psi$ is the parameter that governs how the drift of $p(t)$  changes once we condition on  the
volatility in the immediate future. $\sigma_{\tau+}$ if you will. 


\purple{The other thing to note here is that $\mu_{\sigma}(s)$ is drift of $\sigma^2(s)$ with respect to its own
    filtration. This is not necessarily the same drift as the drift with respect to $\F_t$. The fundamental reason
why $\phi$ and $\psi$ are not necessarily the same is because the drift and volatility do not have to change in the
same way between when we change the measures. Effectively since $\sigma^2(\tau)$ is not a martingale with respect
to its filtration, the drift may change as well. Girsanov's theorem assumes martingales.... ( Or something.)}

\begin{align}
    &\phantom{=} \E\left[r_{t+1} \mvert \F_{t}\right]  - \E\left[\E\left[r_{t+1} \mvert \F_{t+1, \sigma^2} \F_t
      \right] \right] \\
    &= \E\left[\psi \left( \sigma^2_{t+1} - \E\left[\sigma^2_{t+1} \mvert \F_t\right]\right)  \mvert \F_t \right]
       \\
       %TODO I am not quite sure this is right.
    &= \E\left[ \int_t^{t+1} \psi(s) \dif \mu_{\sigma}(s) \mvert \F_t \right]
\end{align}
...
\purple{Why are the $(\cdot)_{\PP, \tau}$ functions continuous functions of $\tau$? Check signs!}
...
\begin{align}
    ...
    \intertext{Then subbing in the formula's for $\phi$ and $\psi$ above and making some notation that is no
    ...
    &=  - \gamma(\tau) - \left[\psi(\tau) + \beta(\tau) \right]
        ...
        \tau + \psi(\tau) \sigma_{\sigma}(\tau) \dif W^{\sigma}
\end{align}


We know that the predictable  quadratic variation of $p(t)$ is $\int_t^{t+1} \sigma^2(s) \dif s$.
In other words, $\psi(\tau) = \phi(\tau)$.

...
We do this by exploiting the fact that $\E_{\PP}\left[M_{t,t+1} \exp(r_{t+1}) \mvert \sigma^2_t \right] = 0$.
...
Having done this we can define a pseudo-return whose mean with respect to the physical measure does not change in
expectation when we add $\sigma^2_{t+1}$ to the conditioning set. 
Let $\tilde{r}_{t+1} \coloneqq r_{t+1} - \frac{1 - \phi^2}{2} \sigma^2_{t+1} - (1 - \phi^2) \theta
\sigma^2_{t+1}$.


\begin{align}
    &\phantom{=} \log \E_{\PP}\left[\widetilde{r}_{t+1} \mvert \sigma^2_t, \sigma^2_{t+1} \right]
%
    \intertext{We can expand $\widetilde{r}_{t+1}$.}
%
    &= \log \E_{\PP}\left[r_{t+1} - \frac{1 - \phi^2}{2} \sigma^2_{t+1} - (1 - \phi^2) \theta \sigma^2_{t+1}
...
%
    &= \log \E_{\PP}\left[\exp(r_{t+1})  \exp\left(- (1 - \phi^2) \theta \sigma^2_{t+1}\right) \mvert \sigma^2_t,
       \sigma^2_{t+1} \right]
%
    ...
    it  as well.}
%
    &\propto \log \E_{\PP}\left[\frac{\exp(r_{t+1})}{\exp(r_t)} \frac{M_{t,t+1}}{M_{t-1,t}} \mvert \sigma^2_t,
      \sigma^2_{t+1} \right]
%
...
Since $\exp(r_{t+1})$ is conditionally log-Gaussian Furthermore, $\log\E_{\PP}\left[\exp(r_{t+1}) \mvert
\sigma^2_{t+1}\right]  = \E_{\PP}\left[r_{t+1} \mvert \sigma^2_{t+1}\right] + \frac{1 - \phi^2}{2}
\sigma^2_{t+1}$.

\begin{equation}
    \E_{\PP}\left[\widetilde{r}_{t+1} \mvert \sigma^2_{t+1}, \sigma^2_t \right]  = \E\left[r_{t+1} \mvert
    \sigma^2_t\right] 
\end{equation}

We know that $\E_{\PP}\left[r_{t+1} \mvert \sigma^2_t, \sigma^2_{t+1} \right] = \psi \sigma^2_{t+1}$ plus some
function  of $\sigma^2_t$.
...
Once we do that  $\E_{\PP}\left[\widetilde{r}_{t+1} \mvert \sigma^2_t, \sigma^2_{t+1} \right] = \widetilde{\psi}
\sigma^2_{t+1}$ plus some function  of $\sigma^2_t$.
$\widetilde{\psi}$ gives us the change in the expectation of $\widetilde{r}_{t+1}$ arising from the leverage
effect.

We still do not know what $\tilde{\psi}$ is, and so we cannot use it to solve for $\psi$.
We turn to solving for it now.
...
\begin{equation}
    ...
    \E_{\PP}\left[\Var_{\PP}\left(\tilde{r}_{t+1} \mvert \sigma^2_t\right)  \right] = \E_{\PP}[ \sigma^2_{t+1}]
\end{equation}
...
This is not the only way we can compute $\Var_{\PP}\left[r_{t+1} \mvert \sigma^2_t \right]$.
...
We can substitute back in the definition of $\tilde{\psi}$ in terms of $\psi$.
%TODO 

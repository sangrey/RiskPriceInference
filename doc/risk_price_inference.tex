...
\begin{restatable}[Log-Price Given Future Volatlity]{theorem}{priceDGPGivenVol}
    ...
    Further assume that all of the prices are locally-square integrable.

    \begin{equation}
            \dif p(t) = \gamma(t) + \beta(t) \sigma^2(t) + \psi(t) \sigma^2(t) + \sigma(t) \sqrt{1 - \phi(t)^2}
            \dif W^p(t) + \phi(t) \sigma(t) \dif W^{\sigma}(t) 
    \end{equation}

    Then we can represent the prices progressively enlarged by $\lim_{\Delta \to 0^{+}} \sigma(t + \Delta0$ for
    each $t$ as follows.
...
Intuitively, $\psi$ is the parameter that governs how the drift of $p(t)$  changes once we condition on  the
volatility in the immediate future. $\sigma^2(t+)$ if you will. 
To see this more fully consider the following.


\begin{equation}
     \E\left[r_{t+1} \mvert \F_{t}\right]  - \E\left[\E\left[r_{t+1} \mvert \F_{t+1, \sigma^2} \F_t
        \right] \right] 
     \E\left[\psi \left( \sigma^2_{t+1} - \E\left[\sigma^2_{t+1} \mvert \F_t\right]\right)  \mvert \F_t \right]
     \E\left[ \int_t^{t+1} \psi(s) \sigma^2(s) \dif (s) \mvert \F_t \right]
\end{equation}
...
\begin{align}
        \dif p(t) = \gamma(t) + \beta(t) \sigma^2(t) + \sigma(t) \sqrt{1 - \phi(t)^2} \dif
         W^{\sigma}(t) + \phi(t)\dif  W^p(t) 
\end{align}

\begin{align}
    &\E\left[ \int^t \phi^2(s) \sigma^2(s)  \dif s \mvert \F_{t-} \right] \\
    %
    = &\E\left[ \predQV*{\int^t \phi(s) \sigma(s) \dif W^p(s)} \mvert \F_{t-} \right] \\
    %
    \intertext{Since $W^{p}(s)$ is independent of $\sigma^2(t)$.} 
    %
    = &\E\left[ \predQV*{\int^t \phi(s) \sigma(s) \dif W^p(s)} \mvert \F_{\sigma^2, t-} \right] \\
    %
    %TODO This could use some more explication.
    \intertext{Since the argument here is still a martingale, it is contained within the diffusion part of $M(t)$.
        The variance cannot have increased because Gaussian's variance does not increase when you condition on
        more information, and because the other part of the process $W^{\sigma}(t)$ is not a martingale in this
        filtration.}
    %
    % = &\E\left[ \predQV*{M^D(t)} \mvert \F_{\sigma^2, t-} \right] \\
    %
    % = &\E\left[ \phi^2(s) \sigma^2(s) \dif s \mvert \F_{t-} \right] 
\end{align}


% In other words, it was no accident, that we are using $\phi(t)$ for two potentially different objects. 
% Effectively, $\phi(t)$ controls the reduction in covariance that is brought about by conditioning on the future
% values of $\sigma^2(t)$.



\begin{equation}
    \dif p(t)  =  \gamma(t) \dif t +  [\beta(t) + \psi(t)] \sigma^2(t) \dif t + \sigma^2(t) \left[\sqrt{1 -
    \phi(t)^2} \dif W^{\sigma}(t) + \phi(t) \dif W^{p}(t)\right]
\end{equation}
...
The problem with \cref{eqn:discrete_time_rtn} is that it allows the various predictable components to vary.
...
\begin{proof}
    ...
    To model the additional information in a tractable way, we consider the following.
    Consider a dense discrete grid of points over $\R$ --- the magnitudes of the prices.
    Define a set of times $\lbrace \tau_i\rbrace $ such that $\tau_i$ is the time at which the volatility crosses
    the $i-th$ point, but not the $(i+1)-th$ one. 
    Note, since equality at a discrete dense set of points is sufficient to imply equality over any open set,
    equality over $\lbrace \tau_i \rbrace$ is sufficient for equality in probability.
    ...
    Define $\F_{\sigma,t} \coloneqq \F_{t+} \cup \sigma\left(\lbrace \tau_i \rbrace\right)$. 
    ...
    integrable, it is without loss of generality to consider the case with only one jump.
    ...
    \right]$.\footnote{Our notation is taken from \textcite{nikeghbali2007nonstopping}, but we simplified slightly
    using the assumptions in our environment.}
    ...
    Note, $\lbrack M(t), \sigma^2(t) \rbrack = 0$ because $\lim_{\Delta \to 0} \sigma^2(t + \Delta)$ is contained
    within the $\F_{\sigma, t-}$ filtration.
    
    \begin{equation}
        \label{eqn:filtration_changed_martingale}
            \int_0^{t \wedge \tau} \sigma(s)  \dif W(s)  = M(t) + \int_0^{t \wedge \tau} \frac{1}{\Pr\left(\tau > s
            \mvert \F_{s-}\right)} \dif \predQV*{\int_0^t \sigma(s) \dif W(s), \mu^{\tau}(t) } (s)  
    \end{equation}
    
    Since $\sigma^2(t)$ has no predictable jumps, $\tau$ avoid all stopping times of $\F_{t-}$.  
    Hence, $\mu^{\tau}(t)$ is continuous \parencite[p. 459]{nikeghbali2005definition}, and so we can rewrite
    \cref{eqn:filtration_changed_martingale} as follows because drifts do not affect quadratic variation.
    ...
    $M(t)$ is still a martingale in $\F^p(t)$, which is the smallest filtration satisfying the standard conditions
    that is generated by the prices augmented with $\F^{t-}$. 
    In addition, since $\predQV{M}(t)$ is predictable, and hence $\F_{t-}$ measurable, we can choose its
    instantaneous variance to be equal in the two different filtrations. 
    We also know that $M(t)$ is orthogonal with respect to $\sigma^(t)$. 
    Consequently, $p(t) \perp \sigma^2(t)$ and $M(t)$ have the same predictable quadratic variation.
    Hence, $p(t) = M(t) + W(t)$, where $W(t) \perp M(t)$.
    $M(t)$ cannot have more because conditioning of Gaussian processes cannot increase the variance, and it cannot
    have less because it is a martingale in the original filtration that forms part of the price.
    Since the prices are continuous, only continuous processes are adapted to $\F^p(t)$.
    Consequently, $M(t)$ is continuous in this filtration.
    
    Then since $M(t)$ is a martingale in $F_{\sigma,t}$ and predictable quadratic variation is predictable,
    conditioning  on more information in this filtration does not change the predictable quadratic variation of
    the diffusion part.
    In addition, the predictable quadratic variation of a martingale with both a diffusion part and a jump part is
    strictly greater than just a diffusion part.
    Hence, we can let $M(t)$ is continuous up to a measure zero set with respect to $\F_{\sigma^2,t}$.

\end{proof}

% \begin{proof}
    
%     We start by characterizing the process in continuous-time and then simplifying it by using a generalized
%     It\^{o}'s lemma.

%     \begin{align}
%         \dif K(u, t) &= \lim_{\Delta \to 0} \log \E_{\PP}\left[\exp\left(-u \int_{t}^{t + \Delta} \dif
%         p(s)\right) \mvert \F_{\sigma^2, t-}\right] \\ 
...
%         &= \lim_{\Delta \to 0} \log \E_{\PP} \left[\int_{t}^{t+\Delta} -u \dif p(s) - \frac{1}{2}
%             \int_{t}^{t+\Delta} -u^2 \dif \langle p(s), p(s) \rangle \mvert \F_{\sigma^2, t-}
%             \right] \\
...
%         &= \E_{\PP} \left[ -u \dif p(t)  + \frac{u^2}{2} \sigma^2(t)  \dif t \mvert \F_{\sigma^2, t-}
%            \right] 
%     \end{align}
...
%     The future volatility --- $\sigma^2_{t}$ --- is a random variable with respect to $\F_{t-}$.
%     We further define $\widetilde{\sigma}^2(t) \coloneqq \sigma^2_{t} - \sigma^2_{t-}$, i.e.\@ the martingale part
%     of $\sigma^2_{t}$.
%     Consequently, we have a series of random variables with respect to $\F_{t}$, one for each $u$.
%     Its predictable compensator $\sigma^2_{t-}$ is measurable with respect to $\F_{t-}$.
%     The price drift with respect to $\F_{\sigma^2, t}$ is in principle a function of both of these variables.


%     \begin{equation}
%         \dif K(u,t) = \E\left[ -u \mu(\widetilde{\sigma}^2(t), \sigma^2(t-)) \dif t - u \sigma \sqrt{1 -
%         \phi^2(t)} \dif W(t) + \frac{u^2}{2} \sigma^2(t) \dif t \mvert \F_{\sigma^2, t-} \right] 
%     \end{equation}

%     By applying It\^{o}'s lemma to $\mu(\sigma^2(t))$, which applies because $\F_{\sigma^2, t}$ contains $\F_{t}$
%     and we are working inside the $\F_{\sigma^2,t}$ measure,  and hence the absolute continuity requirements are
%     met, there exist some predictable functions $\mu_1$ and $\psi$ so that the following holds. 
%     In addition, $\mu_1$ and $\psi$ can be chosen to be  $\F_{\sigma^2, t-}$ measurable and hence $\F_{t-}$
%     measurable. 
%     This follows from It\^{o}'s lemma, since predictable quadratic variation is predictable, and $\psi(t)$ is an
%     appropriately renormalized density of a predictable quadratic variation process.
%     See \parencite[4248]{jeanblanc2015martingale}  for further discussion.
%     In general, we can create such a $\psi$ by multiplying and dividing by $\sigma^2(t)$ which is valid because it
%     is strictly positive.

%     % \begin{alignat}{2}
%     %     &\dif K(u,t) &&= \E\left[ -u \gamma(t) - u \beta(t) \sigma^2(t-) + \mu_1(t) \dif \widetilde{\sigma}^2(t)
%     %       - u \psi(t) \sigma^2(t) - u \sigma \sqrt{1 - \phi^2(t)} \dif W(t) \right. \\
%     %     & &&\quad \left. + \frac{u^2}{2} \sigma^2(t) \dif t \mvert \F_{\sigma^2, t-} ] \nonumber \\ 
%     %       %
%     %       \intertext{Then subbing in for $\dif \widetilde{\sigma}^2(t)$.}
%     %       %
%     %     & &&= \E\left[ -u \gamma(t) - u \beta(t) \sigma^2(t-) - u \mu_1(t) \sigma_{\sigma}(t)\dif W_{\sigma}(t) -
%     %         u \frac{\mu_1(t) \gamma_{\sigma}(t)}{\sqrt{2}} \dif L_{\sigma}(t)   \right.  \\
%     %     & &&\quad -\left. u \psi(t) \sigma^2(t-) - u \sigma \sqrt{1 - \phi^2(t)} \dif W(t) + \frac{u^2}{2}
%     %       \sigma^2(t) \dif t \mvert \F_{\sigma^2, t-} \right] \nonumber \\
%     %       %
%     %       \intertext{For the instantaneous volatility of $p(t)$ to equal $\sigma^2(t)$, $\mu_1(t)
%     %       \sigma^2_{\sigma}(t)$ must equal $\phi(t) \sigma(t)$}
%     %       %
%     %     & &&= \E\left[ -u \gamma(t) - u \beta(t) \sigma^2(t-) - u \mu_1(t) \sigma_{\sigma}(t)\dif W_{\sigma}(t) -
%     %         u \frac{\mu_1(t) \gamma_{\sigma}(t)}{\sqrt{2}} \dif L_{\sigma}(t)   \right.  \\
%     %     & &&\quad -\left. u \psi(t) \sigma^2(t-) - u \sigma \sqrt{1 - \phi^2(t)} \dif W(t) +  \frac{u^2}{2}

...
We construct the following \textquote{return} because it will have useful properties with respect to the
...
\purple{
Since we are using realized volatility to measure variance, we have the following aggregation property
$\E_{\QQ}\left[\sigma^2_{t+} \mvert \sigma^2_t\right] = \E_{\PP} \left[\Var_{\PP}\left(r_{t+1} \mvert
\sigma^2_{t+1}\right) \mvert \sigma^2_t \right]$.\footnote{In continuous-time this is known as the It\^{o}
Isometry.}
\purple{We are also assuming that there is no change in the drift as a function of $\sigma^2_{t+1}$.}
}
...
% \footnote{It is worth noting that  \cref{eqn:return_decomp} has two endogenous

...
\begin{defn}{Continuous-Time DGP}
    ...
    \begin{align}
        \dif p(t) &= \mu(t) \dif t + \sigma(t) \dif W(t)  \\
...
We can separate out the log-price differential into components that depend upon the volatility and independent
components. 

\begin{defn}{Continuous-Time DGP}

    \begin{align}
        \dif p(t) &= \gamma(t) \dif t + \beta(t) \sigma(t-) \dif t + \psi(t) \dif t + \sigma(t) \phi(t) \dif
            W_{p}(t)  + \sigma(t) \sqrt{ 1 - \phi^2(t)} \dif W_{p}(t)  \\
        \dif \sigma^2(s)  &= \mu_{\sigma}(t) \dif t + \sigma_{\sigma}(t) \dif W_{\sigma}(t) 
    \end{align}
...
The $\psi$ and $\phi$ functions above are implicitly defined so that we can represent $\dif p(t)$ with respect to
$\F_{t, \sigma}$ as follows.


\begin{equation}
    \label{eqn:price_given_vol}
    \dif p(t) = \psi(t) \dif t + \sigma(t) \sqrt{1 - \phi^2(t)} \dif W(t)
\end{equation}

\purple{TODO We should probably have a discussion explaining why this is reasonable somewhere.}
As long as $p(t)$ is s still a semimartingale, which we assume throughout, with respect to this filtration, these
functions will exist.
The $\phi(\cdot)$ function governs the correlation between the Brownian motion driving $p(t)$ and the one driving
the volatility.
The $\psi(\cdot)$ function is coming from the fact that once we condition on $\sigma(t+)$ the price drift might
well change.
...
\begin{alignat}{2}
    &\dif K(u,t) &&= \E\left[ -u \mu(\sigma(t)^2) \dif t - u \sigma \sqrt{1 - \phi^2(t)} \dif W(t) + \frac{u^2}{2}
    \sigma^2(t) \dif t \mvert \F_{\sigma^2, t-} \right] \\ 
    \intertext{By applying It\^{o}'s lemma to $\mu(\sigma^2(t))$, which applies because $\F_{\sigma^2, t}$
        contains $\F_{t}$ and we are working inside the $\F_{\sigma^2,t}$ measure,  and hence the absolute
        continuity requirements are met, there exist some predictable functions $\mu_1$ and $\mu_2$ so that the
    following holds.}
    & &&= \E\left[ -u \mu_1(t) \dif \sigma^2(t)  - u \mu_2(t) \sigma^2_{\sigma}(t) - u \sigma \sqrt{1 - \phi^2(t)}
       \dif W(t) + \frac{u^2}{2} \sigma^2(t) \dif t \mvert \F_{\sigma^2, t-} \right] \\ 
       \intertext{Then subbing in for $\dif \sigma^2(t)$.}
    & &&= \E\left[ -u \mu_1(t) \mu_{\sigma} \dif t   - u \mu_1(t) \sigma_{\sigma}(t) \dif W_{\sigma}(t) - u
      \mu_2(t) \sigma^2_{\sigma}(t) - u \sigma \sqrt{1 - \phi^2(t)} \dif W(t) \right. \\ 
    & &&\quad + \left. \frac{u^2}{2} \sigma^2(t) \dif t \mvert \F_{\sigma^2, t-} \right]  \nonumber   \\
    \intertext{For the instantaneous volatility of $p(t)$ to equals $\sigma^2(t)$, $\mu_1(t) \sigma^2_{\sigma}$
        must equal $\phi(t) \sigma(t)$}
    & &&= \E\left[ -u \mu_1(t) \mu_{\sigma} \dif t - u \sigma(t) \phi(t) \dif W_{\sigma}(t) - u \mu_2(t)
       \sigma^2_{\sigma}(t) - u \sigma(t) \sqrt{1 - \phi^2(t)} \dif W(t)  \right. \\
   & &&\quad + \left. \frac{u^2}{2} \sigma^2(t) \dif t \mvert \F_{\sigma^2, t-} \right]  \nonumber 
\end{alignat}


The drift term in the previous equation has the form $\mu_1(t) \mu_{\sigma} + \mu_2(t) \sigma^2_{\sigma}(t) +
\frac{1}{2} \sigma^2(t)$. 
The price process has a drift of the form $\mu_{p}(t) + \beta(p)(t) \sigma^2(t)$.
Clearly, we can write the first drift term as $\mu_{p}(t) + \beta(p)(t) \sigma^2(t) + \psi(t) \sigma^2(t) +
\gamma(t)$ for some functions $\psi, \gamma$.
Of course $\gamma$ and $\psi$ are not unique here, and so we can use projections of the relevant residual onto
$\sigma^2(t)$.

As a consequence, the differential of the log-Laplace transform has the following form. 

\begin{equation}
    \label{eqn:log_cumulant_process}
    \begin{alignedat}{1}
        \dif K(t, u) &= \E\left[ -u [\psi(t) \sigma^2(t) +  \gamma(t) + \beta(t) \sigma^2(t) + \mu_{p}(t)] \dif t
        \vphantom{\frac{u}{2}} \right. \\
        & \quad - \left. u  \phi(t) \dif W_{\sigma}(t)  - u \sigma \sqrt{1 - \phi^2(t)} \dif W(t) + \frac{u^2}{2}
            \sigma^2(t) \dif t \mvert \F_{\sigma^2, t-} \right] 
    \end{alignedat}
\end{equation}
...
can also read off the moments from \cref{eqn:log_cumulant_process}. 

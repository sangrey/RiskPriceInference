...
\declaretheorem[name=Part, style=definition, parent=theorem]{proofpart}
\declaretheorem[name=Corollary, style=definition, parent=theorem]{corollary}
\declaretheorem[name=Lemma, style=definition, sibling=theorem]{lemma}

\declaretheorem[style=definition, name=Definition, refname={definition,definitions}]{defn}
...
\declaretheorem[style=definition, name=Assumption, refname={assumption,assumptions},
    Refname={Assumption,Assumptions}]{assump}
...
\usepackage[backend=biber, autopunct=true, authordate, hyperref=true, doi=false, isbn=false,
...
\begin{theorem}[Inference for $\eta$ under Weak Identification]
...
\begin{assumplist}
    ...
    \item If $\{\gamma_T\} \in \Gamma\left(\gamma_0, 0, b\right)$, $\sup_{\left\lbrace \eta \in \Eta \mvert
        ...
        (\phi, \zeta')'} \sampmom(\eta) - \popmom_{(\phi, \zeta')'}\left(\eta \mvert \gamma_0 \right)} = o_p(1)$
        for all deterministic sequences  $\delta_T \to 0$.
        \label{ass:GMM2b}
    \item[] \purple{(Xu) : Is this a valid translation of the relevant domain?}
    \item If $\{\gamma_T \} \in \Gamma\left(\gamma_0, \infty, \eta_0\right)$, $\sup_{\left\lbrace \eta \in \Eta
        \mvert \norm*{\eta - \eta_0} \leq \delta_T \right\rbrace} \norm*{\left(\frac{\partial}{\partial \eta'}
        \overline{g}_T - g_{\eta}\left(\eta \mvert \gamma_0\right)\right) \diag\left(1_{1+d_\zeta}',
        (1/\phi_T)_{d_{\pi}}'\right)}  = o_p(1)$ for all deterministic sequences $\delta_T \to 0$.
        \label{ass:GMM2c}
\end{assumplist}
\end{assump}

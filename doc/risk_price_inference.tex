...
linear\footnote{We show this in \cref{lemma:linearity_of_physical_functions} which is located in
\cref{app:model_characterization}.}
...
We can now derive forms for $\beta$ and $\gamma$ as functions of the reduced-form parameters and risk-prices.
...
\begin{proof}
    This comes from matching the coefficients of the $\F_t$-measurable variables in the above moment condition and
    using the linearity of $\beta$ and $\gamma$ from \cref{lemma:linearity_of_physical_functions}. 
...
\begin{enumerate}
    \item[Case 1:] The price of equity risk $\theta$ satisfies the following equations. 
        ...
        In this situation, we can satisfy these \cref{lemma:reparamterizing_beta_and_gamma}, regardless of the
        value of $\pi$.  In other words, the asset pricing equation does not identify $\pi$. 
...
If $\phi \neq 0$,on the other hand, the derivatives of \cref{lemma:reparamterizing_beta_and_gamma} do not equal at
$\theta$ and $\theta-1$ since $a$ and $b$ are not linear functions.

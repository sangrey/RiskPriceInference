...
We will do by inducing some functions $\psi(t)$ and $\phi(t)$ which control the drift and volatlity of $p(t)$
under $\F_{\sigma^2,t}$.
To be concrete, we will show that you can represent the log-price with respect to $\F_{\sigma^2,t}$ as follows.

\begin{defn}{$\left. p(t) \mvert \sigma^2(t+) \right.$}
    \label{defn:logPriceGivenVol}

    \begin{equation}
        \dif p(t) = \gamma(t) + \beta(t) \sigma^2(t) + \psi(t) \sigma^2(t) + \sigma^2(t) \sqrt{1 - \phi(t)^2} \dif
        W_{\sigma}(t)
    \end{equation}
...
\begin{defn}[Continuous-Time Log-Laplace Transform]
    \label{defn:cont_time_log_laplace_trans}
    \begin{equation}
        K(u, t) \coloneqq \lim_{\delta \to 0} \log \E_{\PP}\left[\exp\left(-u \int_t^{t+\Delta} \dif p(s) \right)
        \mvert \F_{\sigma^2, t-}\right] 
    \end{equation}
\end{defn}

Since \cref{defn:cont_time_log_laplace_trans} in its current form is not particularly useful, we will
characterize its differential differential.

\begin{restatable}[Characterization of $\dif K(u,t)$]{lemma}{characterizationDifLogLaplace}

    \label{lemma:characterization_of_dif_K}
    For some predictable functions $\psi(t)$ $\phi(t)$ the differential of $K(u,t)$ has the following form.

    \begin{equation}
        \begin{alignedat}{1}
            \dif K(t, u) &= \E\left[ -u [\psi(t) \sigma^2(t) +  \gamma(t) + \beta(t) \sigma^2(t) + \mu_{p}(t)]
                \dif t \vphantom{\frac{u}{2}} \right. \\
            & \quad - \left. u  \phi(t) \dif W_{\sigma}(t)  - u \sigma(t) \sqrt{1 - \phi^2(t)} \dif W(t) +
              \frac{u^2}{2} \sigma^2(t) \dif t \mvert \F_{\sigma^2, t-} \right] 
        \end{alignedat}
    \end{equation}
\end{restatable}

Now, we know that the derivatives of the log-Laplace transform must give moments when evaluated at zero, and we
can also read off the moments from \cref{lemma:characterization_of_dif_K} since the dominated convergence theorem
holds.
Consequently, $\psi(t) = \alpha_{\PP,t}'(0)$, $\phi(t) = \sqrt{-\alpha_{\PP, t}''(0)}$, $\beta(t) =
\beta_{\PP,t}'(0)$, and $\gamma(t) = \gamma_{\PP,t}'(0)$.

\purple{Why is there not a constant term in the alpha equation, comes from the fact that changes of measure are
covariances.}
...
Throughout the following section, I am going to assume that the price process actually has the formula given
above. 
In complete generality it might not.\footnote{If $p(t)$ is continuous, this is not an assumption.}
However, since the prices of the two processes will be the same, this is without loss of generality. 
You can view it as suppressing the process's potential  additional, (non-priced) complexity in the notation. 
Consequently, we can write the log-price process with respect to the $\F_{\sigma^2, t}$ filtration as follows.

\begin{equation}
    \label{eqn:eqn:price_process_decomp}
    \dif p(t)  =  \gamma(t) \dif t +  [\beta(t) + \psi(t)] \sigma^2(t) \dif t +
     \sigma^2(t) \left[\sqrt{1 - \phi(t)^2} \dif W^{p}(t) + \phi(t) \dif W^{\sigma}(t)\right]
\end{equation}
...
\begin{proof}
    ...
    \begin{alignat}{2}
        ...
        & &&\quad \left. + \frac{u^2}{2} \sigma^2(t) \dif t \mvert \F_{\sigma^2, t-} \right] \nonumber \\ 
          %

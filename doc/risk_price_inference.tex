...
\purple{Now, do I think about $\alpha_{\PP}(0)$ as a function of $\tau$?}
...
However, the relative magnitude of their time-variation is not a priori clear.
Equivalently, how fast can the derivatives of the physical measure change over time.  
From semimartingale theory, we know that $\E\left[r_{t+\Delta} \mvert \F_{\sigma^2, \tau}\right]$ is a drift term
with respect to the augmented filtration and hence has stochastic order $\Delta$, as does $\E\left[r_{t+\Delta}
\mvert \F_{\sigma^2, \tau}\right]$ because it is a variance.

\purple{The variance is of the same stochastic order with respect to both filtrations.  I should probably add a
citation for this.}

\purple{TODO: (Paul) Characterize the conditional expectation as an unconditional expectation.}



The key concept that drives our results is as follows.
Since $\sigma^2_{\tau}$ is measurable with respect to the $\F_{\tau}$ filtration $\beta_{\PP}(u)$ and
$\gamma_{\PP}(u)$ terms are drift terms. 
If they were not drift terms, we would not have a well-defined semimartingale as semimartingales only allow
finite-variation in the predictable terms.

However, since $\sigma^2_{\tau + \Delta}$ is not measurable with respect to $\F_{\tau}$, this term can be more
general form. 
Hence, we can replace all of $O_p(\cdot)$ terms in the above expression with the appropriate functions of
$\Delta$. 

\begin{equation}
    - \alpha_{PP}'(0) \sigma^2_{\tau + \Delta} - \alpha_{PP}''(0) \E\left[\sigma^2_{\tau+ \Delta}  \mvert
    \F_{\tau}\right]  - \beta_{\PP}'(0) \sigma^2_{\tau} - \gamma_{PP}'(0)  + o(\Delta) \\
\end{equation}

Simplifying the notation slightly and suppressing asymptotically negligible terms, we can rewrite that expression as
follows.

\begin{equation}
    - \alpha_{PP}'(0) \sigma^2_{\tau + \Delta} - \alpha_{PP}''(0)  \sigma^2_{\tau} - \beta_{\PP}'(0)
    \sigma^2_{\tau} - \gamma_{PP}'(0)  \\
\end{equation}



Similar to the argument in \cref{sec:deriving_mom_conds} we want to replace that as measurable with respect to the
augmented filtration with one that is measurable with respect toe he prices natural filtration. 
We can do this because one filtration contains the other and so absolute continuity requirements are automatically
satisfied. 
By Girsanov's theorem  and the fact that the second two terms are measurable with respect to both filtrations the
process with respect to $\F_{\tau}$ has the following form.

\begin{align}
    &= - \alpha_{PP}(\tau) \dif W^{\sigma}(s) + \frac{1}{2} \alpha_{\PP}(\tau-) \sigma^2(\tau-) -
       \beta_{\PP}(\tau) \sigma^2_{\tau-} - \gamma_{PP}(\tau) \\
   \intertext{We can absorb both coefficients for the $\sigma^2(\tau-)$ term into one without loss of generality.}
   \label{eqn:continous_time_log_laplace}
    &= - \alpha_{PP}(\tau-) \dif W^{\sigma}(s) +  \beta_{\PP}(\tau) \sigma^2_{\tau-} - \gamma_{PP}(\tau) \\
\end{align}

From the decomposition in in \cref{eqn:continous_time_log_laplace}  we can see where the volatility and return can
be related is through the correlation between $W^{\sigma}(\tau)$  and the risk-price.

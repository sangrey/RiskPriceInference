...
The last two equations are not moment conditions per-se. 
Rather they are best understood as 
...
%
    \Var\left[\sigma^2_{t+1} \right]  &=  \frac{c^2}{1 - \rho^2}  \left(\frac{2 \rho \delta}{1 - \rho}  + \omega
        \right)  \\
%
    \E\left[r_{t+1} \mvert \sigma^2_t\right] &= \gamma + \beta \sigma^2_t + \sigma_{t+1} \left(\phi e_{1,t}  +
        \sqrt{1 - \phi^2} e_{2,t}\right) + e_{3,t} \\
%
    \E\left[r_{t+1} \mvert \sigma^2_t, \sigma^2_{t+1}\right] &= \gamma + (\beta + \psi) \sigma^2_{t+1}  + \sigma_{t+1}
    \sqrt{1 - \phi^2}  e_{2,t} + e_{2,t} \\
%
        % \mathrm{Corr}(e_{1,t}, e_{2,t}) &= \phi \\
%
    \gamma (\theta - 1) + c \delta \left(\pi + \psi(\theta - 1) + \phi \frac{(\theta - 1)^2}{2}\right) &= \gamma
    \theta + c \delta \left(\pi + \psi \theta + \phi \frac{\theta^2}{2} \right) \\ 
%
...
As for instruments, we use the lags of volatility. 
\begin{defn}{Instruments}
    \label{defn:instruments}
    \begin{equation}
        Z_t \coloneqq 1, \sigma^2_{t}, \sigma^2_{t-1}, \sigma^2_{t-2}, \ldots
    \end{equation}
\end{defn}

...
To interpret these factors, we derive a series of moments in \cref{remark:vol_moment_conditions}. 
...
\begin{align}
    ...
    \E_{\PP}\left[r_{t+1} \mvert \F_t\right] &= \psi_{\PP} \E\left[\sigma^2_{t+1} \mvert \F_t\right]  + \beta_{\PP}'(0)
...
kurtosis should not be affected in the risk-prices, and since we are only interested in $\psi_{\PP}$ with regard to its
...
%
    \implies \psi_{\PP} &= \frac{\E_{\PP}\left[\Var_{\PP}\left[r_{t+1} \mvert \sigma^2_t\right]\right] -  (1 - \phi^2)
...
As discussed in \textcite[13]{khrapov2016affine}, since $\psi_{\PP}$ is a adjustment to a drift, if $\phi=0$, (there is
...
To see this more fully, note that the volatility risk price is being identified through the part of $\psi_{\PP}$ above

...
We start by deriving the first two conditional moments of $r_{t+1}$.
These formulas disagree with those in \textcite[5]{khrapov2016affine} because the Laplace transform is a
generating functions for the moments, not the centered moments.   
Their derivation is incorrect.

\begin{align}
    \E\left[r_{t+1} \mvert \F_t, \sigma^2_{t+1}\right]  &= \alpha_{\PP}'(0) \sigma^2_{t+1}  + \beta_{\PP}'(0)
    \sigma^2_{t} - \gamma_{\PP}'(0) \\
    \E\left[r_{t+1}^2 \mvert \F_t, \sigma^2_{t+1}\right]  &= -\alpha_{\PP}''(0) \sigma^2_{t+1}  - \beta_{\PP}''(0)
    \sigma^2_{t} - \gamma_{\PP}''(0)
\end{align}

We can then solve for the conditional variance of $r_{t+1}$.
To understand the intuition, we would generally expect the second derivatives of the various functions to be
negative  here.

\begin{equation}
    \Var\left[r_{t+1} \mvert \F_t, \sigma^2_{t+1}\right]  = - \left[\alpha_{\PP}''(0) + {\alpha_{\PP}'(0)}^2\right]
    \sigma^2_{t+1} - \left[\beta_{\PP}''(0) + {\beta_{\PP}'(0)}^2\right] \sigma^2_t - \left[\gamma_{\PP}''(0) +
    {\gamma_{\PP}'(0)}^2\right]
\end{equation}
...
\begin{equation}
    \begin{alignedat}{2}
        \Var\left[r_{t+1} \mvert \F_t\right]  &= &&-\left[\alpha_{\PP}''(0) + {\alpha_{\PP}'(0)}^2\right]
            \E\left[\sigma^2_{t+1} \mvert \F_t\right] - \left[\beta_{\PP}''(0) + {\beta_{\PP}'(0)}^2\right]
            \sigma^2_t - \left[\gamma_{\PP}''(0) + {\gamma_{\PP}'(0)}^2\right]  \\
        & &&+ \alpha_{\PP}''(0) \Var\left[\sigma^2_{t+1} \mvert \F_t\right]
    \end{alignedat}
\end{equation}

Clearly, we need to compute $\Var\left[\sigma^2_{t+1} \mvert \F_t\right]$.
Again, we differentiate the relevant conditional Laplace transform.


\begin{align}
    \E\left[\sigma^2_{t+1} \mvert \F_t \right]  &= a_{\PP}'(0) \sigma^2_{t}  + b_{\PP}'(0) \\
    \E\left[\sigma^4_{t+1} \mvert \F_t \right]  &= -a_{\PP}''(0) \sigma^2_{t}  - b_{\PP}''(0) \\
    \Var\left[\sigma^2_{t+1} \mvert \F_t \right]  &= - \left[a_{\PP}''(0) + {a_{\PP}'(0)}^2\right] \sigma^2_{t}  -
        \left[b_{\PP}''(0) + {b_{\PP}'(0)}^2\right]
\end{align}



That implies that the variance of $r_{t+1}$ is as follows.
